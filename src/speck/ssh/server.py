...
class SpeckSSHServer(asyncssh.SSHServer):
    def __init__(self, world, command_queue):
        self._world = world
    ...
    def session_requested(self):
        return SpeckSSHSession(self._world, self._command_queue)
...
class SpeckSSHSession(asyncssh.SSHServerSession):
    ...
    def _handle_command(self, line):
        if line == 'agents':
            ...
            parts = line.split(' ', 2)
            if len(parts) == 3:
                self._chan.write(cmd_write(self._command_queue, parts[1], parts[2]) + '\r\n> ')
            else:
                self._chan.write('usage: write <key> <value>\r\n> ')
        elif line.startswith('timewarp '):
            parts = line.split(' ', 1)
            if len(parts) == 2:
                try:
                    value = float(parts[1])
                    self._command_queue.put(("timewarp", value))
                    self._chan.write(f'timewarp set to {value}\r\n> ')
                except ValueError:
                    self._chan.write('usage: timewarp <value>\r\n> ')
            else:
                self._chan.write('usage: timewarp <value>\r\n> ')
        elif line == 'help':
            self._chan.write('commands: agents, write <key> <value>, timewarp <value>, help\r\n> ')
...
def start_ssh_server(world, command_queue, host='127.0.0.1', port=2222):
    def run():
        ...
        async def _start():
            ...
            await asyncssh.create_server(
                lambda: SpeckSSHServer(world, command_queue),
                host, port,
